...
PageDock is a local-first Windows and Apple Silicon macOS PDF study and technology-research workspace for an individual user. It should remain useful without AI, require no development tools from recipients, and keep every person's library, accounts, credentials, notes, and analyses on that person's computer.
...
- Data stays local and backups are portable across Windows and macOS paths.
...
Out of scope for 0.4: company data or internal models, FTO/legal conclusions, cloud collaboration, bulk crawling, embeddings, weekly radar, technology maps, automatic terminology dictionaries, and WebView2 migration.
