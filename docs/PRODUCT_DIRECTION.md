...
- The active Library stays on the Windows local filesystem. Backups remain portable when a user changes PCs or library paths; a NAS or synced folder may hold only verified, passive snapshots and read-only projections, never live Library state.
