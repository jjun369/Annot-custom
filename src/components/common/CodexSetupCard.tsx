...
import { CheckCircle2, Download, ExternalLink, Loader2, LogIn, RefreshCw } from 'lucide-react';
...
export function CodexSetupCard({ compact = false, onStatusChange }: CodexSetupCardProps) {
  ...
  const runAction = async (action: 'install' | 'update' | 'login') => {
    ...
    try {
      ...
      setStatus(next);
      onStatusChange?.(next);
      setMessage(action === 'login' ? 'Codex 로그인 상태를 확인했습니다.' : 'Codex 준비가 완료되었습니다.');
    } catch (error) {
  ...
  const setupUrl = status?.setupUrl || 'https://chatgpt.com/codex';
    ...
    <div className={`rounded-xl border border-outline-variant/25 bg-surface ${compact ? 'p-4' : 'p-5'}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 flex-wrap gap-2">
          {!status?.installed && status?.canAutoInstall && (
            <button
              type="button"
          ...
          {!status?.installed && status && !status.canAutoInstall && (
            <a
              href={setupUrl}
              target="_blank"
              rel="noreferrer"
